...
def dodge(direction: str):
    app.BASE_POSITION[2] += 10
    return 

def handle_event(id: str, details: string):
    # print(f"[debug] handling event {id}, {details}")
    # print(f"[info] handling event {id}, {details['source']}->{details['deliver_to']}: {details['operation']}")
    delivery_required = False
    try:
        if details['operation'] == 'position-error':
            details['deliver_to'] = 'gps-position'
            details['operation'] = 'position-response'
            delivery_required = True
            
        elif details['operation'] == 'position-change':
